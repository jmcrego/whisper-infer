...
if __name__=='__main__':
    ...
    parser.add_argument('--hyp', type=str, required=True, help='File with raw hypotheses.')
    parser.add_argument('--ref', type=str, required=True, help='File with raw references.')
    parser.add_argument('--ch', type=int, default=1, help='Channel to use when stereo audio.')
    ...
    hyps, refs = h2r(args.hyp, args.ref, args.ch)
    wer = jiwer_wrap(show_alignments=args.alignments, uppercase=args.uppercase, no_punct=args.no_punct, no_hesit=args.no_hesit, no_noise=args.no_noise, split_apos=args.split_apos, single_line=args.single_line)
    measures = wer(hyps, refs)
    print(measures)
