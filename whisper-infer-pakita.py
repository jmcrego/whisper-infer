...
if __name__ == '__main__':
    ...
    logging.basicConfig(format='[%(asctime)s.%(msecs)03d] %(levelname)s %(message)s', datefmt='%Y-%m-%d_%H:%M:%S', level=getattr(logging, 'INFO' if args.verbose else 'WARNING'), filename=None)
    ...
    for i in range(len(audios)):
        w = infer(args.model, audios[i], load=load, split_stereo=True)
        for ch in range(2):
