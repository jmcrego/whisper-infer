...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', type=str, default=None, required=False, help='File where transcriptions are output (leave empty for STDOUT).')
    ...
    fdo = open(output, 'w') if output is not None else sys.stdout
    transcribe = json.loads(args.transcribe) if args.transcribe is not None else {}
    w(args.audio, fdo=fdo, split_stereo=args.split_stereo, transcribe=transcribe)
